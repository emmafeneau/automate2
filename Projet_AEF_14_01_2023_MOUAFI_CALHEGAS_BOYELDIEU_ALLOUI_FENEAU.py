...
#saisir un aef pour la fonction produit de deux aefs
def saisir_AEF_produit():
    ...
    for i in range(n):
        etat = {}
        #ajout de l'index pour la fonction produit pour se rappeler de l'état sur lequel on travaille
        etat["index"] = i
        print(f"État {i} :")
        for alphabet in alphabets:
            transitions = input(f"États atteints avec '{alphabet}' (séparés par des espaces) : ").split()
...
def produit_AEF():
    ...
    #saisir le premier aef
    print("veuillez saisir le premier aef\n")
    AEF_1 = saisir_AEF_produit()
    print("vous avez saisi le premier AEF\n")
   
    for i, etat in enumerate(AEF_1):
                print(f"{i} = {etat}")

    #saisir le deuxième aef
    print("veuillez saisir le deuxième aef\n")
    AEF_2 = saisir_AEF_produit()
    print("vous avez saisi le deuxième aef\n")
                ...
                for transition_1 in etat_1:
                    ...
                    for transition_2 in etat_2:
                        if transition_2 == "initial" or transition_2 == "final" or transition_2 == "index":
                            continue
                        #recherche des transitions identiques 
                            ...
                            if len(etat_1[transition_1]) > 0 and len(etat_2[transition_2]) > 0:
                                ...
                                for item_1 in etat_1[transition_1]:
                                    ...
                                    for item_2 in etat_2[transition_2]:
                                        etat_p_1 = []
                                        #sauvegarde des prochains états à parcourir
                                        etat_suivant_1 = item_1
                                        etat_suivant_2 = item_2 
                                        #ajout des prochains états dans la liste de la transition                 
                                        etat_p_1.append(etat_suivant_1)
                                        etat_p_1.append(etat_suivant_2)
                                        etat_p[transition_1].append(etat_p_1)

                #on ajoute dans l'aef_produit l'état initial                      
                AEF_P.append(etat_p)
            #gestion des etats intermédiaires et finaux
                ...
                #on ajoute dans l'aef_produit le nouvel état
                AEF_P.append(etat_p)
...
if __name__ == "__main__": 
    while True: 
        ...
        print("14. Rendre un AEF émondé")
        print("15. Faire le produit de deux AEFs")
        print("0. Quitter")
